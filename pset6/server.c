...
// file descriptor for sockets          //cfd client file discriptor pointer to my we client // sfd server file discripter pointer to web server
int cfd = -1, sfd = -1;
...
int main(int argc, char* argv[])
{
    ...
    int opt;
    while ((opt = getopt(argc, argv, "hp:")) != -1) // getopt - pick anything start w "-" or "--"on argv return -1 means no more option//
    {
        switch (opt)
        {
            // -h
            case 'h':
                printf("%s\n", usage);              //if you see h, print usage coz it means help//
                return 0;

            // -p port
            case 'p':
                port = atoi(optarg);                //option argument which probably port number//
                break;
        }
    }

    // ensure port is a non-negative short and path to server's root is specified
    if (port < 0 || port > SHRT_MAX || argv[optind] == NULL || strlen(argv[optind]) == 0) //optind - next index of argv (if it7s option or not//)
    {
        // announce usage
        printf("%s\n", usage);
    ...
    // start server
    start(port, argv[optind]);          // line 850 //pass into the port as well as any option inorder to datrt on the port//

    // listen for SIGINT (aka control-c)        //event lisener 
    struct sigaction act;
    act.sa_handler = handler;                   // anything hit by user run handler function
    act.sa_flags = 0;
    ...
    char* message = NULL;
    size_t length = 0;                  // size_t means int//

    // path requested
    char* path = NULL;

    // accept connections one at a time             // webserver looping infinitely
    while (true)
    {
        ...
        if (cfd != -1)                      //cfd defined on line 64 //
        {
            close(cfd);                     //close file descripter <unistd.h>//
            cfd = -1;                       //means no file opened??/// close success fully should return to 0
        }

        // check for control-c
        if (signaled)                       // line 70 set as false//
        {
            stop();
        }

        // check whether client has connected
        if (connected())                    //if it is connected//
        {
            // check for request
            if (request(&message, &length)) // looking at top and bottom left and right. put all of them into message//
            {                               //&message - address of pointer ** - see line684
                // extract message's request-line
                // http://www.w3.org/Protocols/rfc2616/rfc2616-sec5.html
                const char* haystack = message;                 // these 2 lines are useful for parse//
                const char* needle = strstr(haystack, "\r\n");  //serching for the end of the line so I can read the one line into memory//
                if (needle == NULL)
                {
                    error(500);
                    continue;
                }
                char line[needle - haystack + 2 + 1];           //allocate enough memory for the line//
                strncpy(line, haystack, needle - haystack + 2); //copy the one line 
                ...
                if (parse(line, abs_path, query))       //take the very first line, extract absolute path, c
                {                                   //interate line character for chara for chara  to look for abs_path and query (check video again)
                    // URL-decode absolute-path     /////////////////////////////here!!
                    ...
                    strcpy(path, root);  
                    strcat(path, p);    // put together to make complete pathto the file//
                    free(p);
                    ...
                    const char* type = lookup(path); ////////////////////////////////////here!!!
                    if (type == NULL)              // check what sort of MINE if php look there otherwise just pass//
                    {
                        error(501);
                        continue;
                    }
...
char* indexes(const char* path) 
{
    // TODO
    /*
     Path is root + abs_path that which is a complete path
     if index.php exists return path
     the same for html otherwise retun NULL
        check if it exists:
        char* file_name = "/index.php";
        int result = access (file_name, F_OK)
        if succeed return 0 otherwise return -1
     
     so first

    make char *php_file (or html check) allocationg memory
    int result by checking using access and F=OK
    if so, return 0 then return to php_index or return NULL
    */
    
    char* php_index = malloc (sizeof (char)* (strlen (path) + 11));
    char* html_index = malloc (sizeof (char)* (strlen (path) + 12));
    
    char* file_php = "/index.php";
    char* file_html = "/index.html";
    
    strcpy (php_index, path);
    strcpy (html_index, path);
    
    strcat (php_index, file_php);
    strcat (html_index, file_html);
    
    int result;
    
    if ((result = access (php_index, F_OK)) == 0) 
    {
        return php_index;
    }
    
    if ((result = access (html_index, F_OK)) == 0)
    {
        return html_index;
    }
...
void interpret(const char* path, const char* query)
{
    ...
    if (access(path, R_OK) == -1)               //access success return0; fail return-1; R_OK means readable//
    {
        error(403);
        return;
    }
...
{
    // TODO
    /*file is already opened and read mode
    char* content;
    defined size_t lenght; in imterpret
    load (file, &content, &length) in imterpret
    size_t means int
    BITE is char defined a bit 
    read all available bytes from the file
    stores these bytes on heap
    stores the first address on *content
    stores number of bytes at * length
    
    content is pointer to pointer
     let's make buffer
     we have the address of content which value is the adress of the first address of buffer
     
     malloc c_data 
     int i for array number and length
     loop until eof keep fgetc (file)
     char c = fgetc (file)
     c will be pasing each bit (char) to char c_data [i]
     once done then 
     * content = c_data
    ...
    */
    ...
    for (int c = fgetc (file); c != EOF; c=fgetc (file))        
    {
        c_data [i] = (char)c;
        i++;
        c_data = realloc (c_data, sizeof(char)*(i+1));      
    }
  ...
  /*
  char* data;
  int i = fread(buffer, BYTE, 1, file)
  while it,s 1
  c_data = buffer[]
...
// to do
{
    char* MimeType = malloc (sizeof(char)*6);                          // array need include \n
    
    MimeType = strchr (path, '.');              // strrchr is including null charactor 
                                                // you can also just put '.'
    
    if (MimeType != NULL)
    {
        if (strncasecmp (MimeType, ".css", 4) == 0)
            return "text/css";                              
        else if (strncasecmp (MimeType, ".html", 5) == 0)
            return "text/html";
        else if (strncasecmp (MimeType, ".gif", 4) == 0)
            return "image/gif";
        else if (strncasecmp (MimeType, ".ico", 4) == 0)
            return "image/x-icon";
        else if (strncasecmp (MimeType, ".jpg", 4) == 0)
            return "image/jpeg";
        else if (strncasecmp (MimeType, ".js", 3) == 0)
            return "text/javascript";
        else if (strncasecmp (MimeType, ".php", 4) == 0)
            return "text/x-php";
        else if (strncasecmp (MimeType, ".png", 5) == 0)
            return "image/png";
        else 
            return NULL;
    }
...

{
    ...
    if (strncmp (line, "GET ", 4) != 0)         // if method is GET and 4th one is space
    {
        error (405);
        return false;
    }
    ...
    if (strncmp (&line[4], slash,1) != 0)       // if tatrget request is not start with /
    {                                           // strcmp (char * char*)
        error (501);
        return false;
    }
    //char* Start_TR = line + 4;                            // strchr (chr*, int) int is unsigned char
    char* Start_TR = strchr (line, '/');                    // find start of target request 
    char* End_TR = strchr (Start_TR, ' ');                  // find end of target request string
    
    char* HTTP_vers = End_TR + 1;                           // skip the space End_TR point
    char target_request [End_TR - Start_TR];                // detarmine target request array size including \0
    
    strncpy (target_request, Start_TR, End_TR - Start_TR);  // Target request copied from line with correct array number
    target_request [End_TR - Start_TR] = '\0';
    
    if (strchr (target_request, '"') != NULL)              // if target request contents "
    {
        error (400);
        return false;
    }
    
    if (strcmp (HTTP_vers, "HTTP/1.1\r\n") !=0)
    {
        error (505);
        return false;
    }
    
    char* Start_q = strchr (line, '?');       // if target request contents ?
    //memset (abs_path, '\0', LimitRequestLine + 1) ; 
    //memset (query, '\0', LimitRequestLine + 1);
    if (Start_q == NULL)
    {
                                      // do i nees memset?
        strcpy (abs_path, target_request);          
    ...
    {
        char path [Start_q - Start_TR] ;                    // determine size of path
        strncpy (path, Start_TR, Start_q - Start_TR);       // copy onto path
        path [Start_q - Start_TR] = '\0';                 // null temination
        strcpy (abs_path, path);                            // copy to abs_path
       
        
        Start_q = Start_q+1;                                // Star_q now point to 'q' or null terminater if there is none
        char query_temp [End_TR - Start_q];                 // detarmine size of query_temp
        strncpy (query_temp, Start_q, End_TR - Start_q);    // copy onto query_temp
        query_temp [End_TR - Start_q] = '\0';               // null termination
        strcpy (query, query_temp);                       //copy to query
